...
- 简而言之，SDS 选择对各个扩散程度的样本都进行采样，这样做的好处是对整个优化过程引入“平滑性”，因为当 $t$ 比较大的时候，离最终结果还比较远，$p_{t}(x_{t}\midy)$ 分布会相对平滑，梯度更稳定，更容易优化；但问题是，它会导致生成的样本过于平滑，以至于丢失部分细节
	- 其实理想情况下，我们只需要对 $p_{t=0}(x_{0}\midy)$ 这个分布优化即可
